...
'''def rolepage1(request):
    ...
    if request.POST:
        ...
        obj = Courselevelusers.objects.filter(instituteid__instituteid=institute_id).filter(personid=request.session['person_id']).values("roleid").distinct()
        ...
        context={'flag':False,'rolelist':rolelist}    
        #print context
        return render(request,'rolepage1.html',context)    
    

                 
    l=[]  
    person_obj = Personinformation.objects.get(email=request.session['email_id'])
    request.session['person_id']=person_obj.id#person id set
    
   
    insti_obj=Institutelevelusers.objects.filter(personid=person_obj.id).values("instituteid").distinct()
...
#---------------------------------------update-------------------------------------------------------------------
    insti_obj=Courselevelusers.objects.filter(personid=person_obj.id).values("instituteid").distinct()
...
#---------------------------------------update-------------------------------------------------------------------
    ...
    return render(request,'rolepage1.html',dic)    
        

#-----------------------------------------------------------------

def rolepage2(request,role):
        print role    
        request.session['role_id']=int(role)#role id set
        print request.session['role_id'],"naya role"
...
def resetpass(request,emailid):
    ...
    if request.method == 'POST':
        ...
        args = {}
        args.update(csrf(request))
        args['password1']= request.POST.get('new_password1','')
        args['password2']= request.POST.get('new_password2','')
        args['message']=[]
        args = pwd_field_empty(request,args,'')
        if args['message']:
            return render_to_response('login/resetpass.html',args)
## Encrypting the password       
        pwd1 = make_password(request.POST.get('new_password1',''),salt=None,hasher='pbkdf2_sha256')
        pwd2 = request.POST.get('new_password2','')
## Displays an error message if the two new passwords don't match
        if not check_password(request.POST.get('new_password2',''), pwd1):
            args = {}
            args.update(csrf(request))
...
def home(request):
        args={}
        args.update(csrf(request))time
        return render_to_response('login/home.html',args)
   
## This function allows user to change his account password if he forgets it
...
def login_success(request):
    #context = RequestContext(request)
     args = {}
     args.update(csrf(request)) 
     insti_obj = T10KT_Institute.objects.get(institutimeteid=request.session['institute_id'])
...
## This function allows user to change his password once he logs into his account  
def change_pass(request):
    ...
    if request.method == 'POST':
        ...
        args['password2']= request.POST.get('new_password2','')
        args['message']=[]
        args = pwd_field_empty(request,args,'CH_PASS')
        if args['message']:
...
## If the old password does not match with his emailtime id, an error message is displayed
        if flag == 0:
            args = {}
            args.update(csrf(request))
...
## If the old password and the new password is the same, an error message is displayed
        elif check_password(oldpwd, pwd1):
            args = {}
            args.update(csrf(request))
            args['message']= retrieve_error_message(module,page,'MTCH_ERR')
            return render_to_response('login/changepass.html',args)

## If the two new passwords don't match, an error message is displayed
        elif not check_password(pwd2,pwd1):
            args['message']= retrieve_error_message(module,page,'PWD_NO_MTCH')
            return render_to_response('login/changepass.html',args)

## Following code changes the password of a user
        else:
            password_reset(request.session['person_id'],pwd1)
            ec_id = EmailContent.objects.get(systype='Login', name='changepass').id        
            id = request.session['person_id']
            per_obj = Personinformation.objects.get(id = id)
            fname = per_obj.firstname
            email = per_obj.email
            mail_obj = EmailContent.objects.filter(id=ec_id)
            message = mail_obj[0].message %(fname)    
            send_mail(mail_obj[0].subject, message ,time EMAIL_HOST_USER ,[email], fail_silently=False)  
            args = {}
            args.update(csrf(request))
            logoutt(request)
            return render_to_response('login/change_pwdsuccess.html',args)
...
## This function allows user to reset his password when he clicks on the personalised link sent to him



    
## This function logs out a user and clears his data  
def logout(request):
    logoutt(request)
    args = {}
    args.update(csrf(request))
...
def studentdetails(request,courseid,pid):
    ...
    #course = courseid argument for teacher related course
    #personid = 0
    #if int(pid) == 0:
    #    personid=request.session['person_id']  #personid(or teacherid) from session
    #else:
      #  personid = pid
    #print pid,personid,request.session['person_id']
    ...
    for student in students:   #iterate over objects of studentdetails
		try:
			print student.edxuserid.edxuserid
    ...
    #context = {'info':data,'course':course,'courseargs':courseargs,'id':pid}    #context of required information
    args['info']=data
    args['courseid']=courseid
    #args['courseargs']=courseargs
    args['id'] = pid
...
def studentdetails(request,course,pid):
    args = {}
    args.update(csrf(request))
    person=Personinformation.objects.get(email=request.session['email_id'])
    args['firstname']=person.firstname
    ...
    if int(pid) == 0:
        ...
        personid = pid
    #print pid,personid,request.session['person_id']
    students = studentDetails.objects.filter(teacherid_id=personid,courseid=course)#select all students who belong to this teacher and course
    data=[]    #list to append data
    for student in students:   #iterate over objects of studentdetails
           #append required data
           data.append([student.roll_no,student.userid.username,student.userid.email,student.id])
...
def addstudent(request,pid):
        if request.method == "POST":
                ...
                emailid = False
                for student in students:
                        userstr = str(student.userid.username)
                        if userstr == username:
                                user = True
                                #print type(student.userid.email),type(email)
                                if student.userid.email == email:
                                        #print "hello"
                                        if int(pid) == 0:
                                                studentDetails.objects.filter(userid__username = username).update(teacherid_id   =request.session['person_id'],roll_no = roll_no)
                                        else:
                                                studentDetails.objects.filter(userid__username = username).update(teacherid_id=int(pid),roll_no = roll_no)
                                        teacherid = 0
                                        if int(pid) == 0:
                                                teacherid = request.session['person_id']
                                        else:
                                        ...
                                        #send_mail(mail_obj[0].subject, message , EMAIL_HOST_USER ,[email], fail_silently=False)
                                        #send_mail(mail_obj2[0].subject, message2 , EMAIL_HOST_USER ,[teacheremail], fail_silently=False)
                                        return HttpResponseRedirect('/studentdetails/IITBombayX/CS101.1x/2015-16/%d'%((int(pid))))#take course id from session
...
def form1(request):
        ...
        if request.method=="POST":
                ...
                x=Userlogin(email=request.POST.get('email'),password=pwd1)
                x.save()
