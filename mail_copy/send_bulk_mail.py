...
for line in reader:
    if line:
        ...
        try: 
            ...
            mail_obj= mail_interface(fname = line[0], lname = line[1], email = line[2], instituteid= T10KT_Institute.objects.get(institutename = line[3]), role = line[4], course = line[5], error_message = error)        
            mail_obj.save()
            section = 2
        except:
...
if section == 2:   
        ...
        for row in SIP_mail_interface.objects.all():
            if row.role == 'Head of Institute':    #Save data to Approvedinstitute if the person is HOI
                appins_obj = SIP_t10kt_approvedinstitute.objects.get(instituteid = row.instituteid)
                if appins_obj.heademail != '':
                    ...
                    appins_obj.save()
                    print "done"
                    ec_id = SIP_emailContent.objects.get(systype = 'TM_MGMT_PC', name = 'register').id
                    mail_obj = SIP_emailContent.objects.filter(id=ec_id)
