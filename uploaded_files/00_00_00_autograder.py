...
def generate_testcases(inputs, outputs, ref_src, target_exe,student_key) :
    ...
    # generate it by compiling and running ref_src
        ...
        if not comp_status == 0:
            ...
            raise Exception
        """
        logfile = open("/tmp/cs101_"+student_key+".py", 'w+')
        
        logfile.write(ref_src)
        for testcase in testcases :
            (stdout, stderr, rcode) = run__code( testcase[1], "", 100,student_key)
            print (stdout, stderr, rcode)
...
def run_code( std_input, expected_output, timeout,student_key) :
    
    process = subprocess.Popen(["python","/tmp/cs101_"+student_key+".py"],
...
def eval_student_submission(sub_src, block_info, target_exe, testcases, timeout, test_freq, out_file, err_file,student_key) :
    
    block_score_list = []
    
    for block_id in range(1, len(block_info) + 1) :
        #print block_info[block_id],"hello",block_id
        block_score = 0
...
"""
        ...
        for testcase in testcases :
            ...
            for run in range(test_freq) :
                ...
                (out, err, status) = run_code(testcase[1], testcase[2], timeout,student_key)
                if status == 0 : break
...
if __name__ == "__main__":
    ...
    if not options.student_key :
        ...
        raise Exception
    target_exe = "/tmp/cs101_" + options.student_key + ".py"
    ...
    testcases = generate_testcases(options.inputs, options.expected_outputs,
                                                        ref_src, target_exe,options.student_key)
    ...
    block_score_list = eval_student_submission(sub_src, block_info, target_exe, testcases,
                                               ...
                                               sys.stdout, sys.stderr,options.student_key)
